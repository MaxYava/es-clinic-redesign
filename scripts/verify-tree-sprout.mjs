...
for (const mobile of [false,true]) {
 ...
 const root = page.locator('section[aria-labelledby="tree-sprout-title"]');
 ...
 for (const progress of [0,.45,1]) {
  ...
  await page.screenshot({path:`artifacts/tree-sprout-${mobile?'mobile':'desktop'}-${Math.round(progress*100)}.png`});
