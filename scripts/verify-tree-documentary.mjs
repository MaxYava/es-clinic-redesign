...
for (const mobile of [false,true]) {
 ...
 const root = page.locator('section[aria-labelledby="tree-documentary-title"]');
 const box = await root.evaluate(el=>({y:el.getBoundingClientRect().top+scrollY,h:el.offsetHeight}));
 for (const progress of [0,.45,.95,1]) {
  await page.evaluate(({box,progress})=>scrollTo({top:box.y+progress*(box.h-innerHeight),behavior:'instant'}),{box,progress});
  await page.waitForTimeout(1000);
  await page.screenshot({path:`artifacts/tree-documentary-${mobile?'mobile':'desktop'}-${Math.round(progress*100)}.png`});
  const ready = await root.locator('[data-ready="true"]').count();
  if (!ready) throw Error('Tree frame not drawn');
  const frameProgress = await root.evaluate(el=>Number(el.style.getPropertyValue('--progress')));
  if (progress >= .95 && frameProgress < 1) throw Error('Final frame did not hold');
 }
